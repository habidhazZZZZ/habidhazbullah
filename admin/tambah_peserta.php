...
<body>
    ...
    <div class="admin-container form-container">
        ...
        <?php if ($message): ?>
            ...
            <?php if ($new_token):
                // Membuat URL dasar yang menunjuk ke root direktori proyek
                $base_url = "http://" . $_SERVER['HTTP_HOST'] . rtrim(str_replace('/admin', '', dirname($_SERVER['PHP_SELF'])), '/\\');
                $pretty_url = $base_url . "/tes/" . $new_token;
            ?>
            <div class="token-info">
                <strong>Link Tes untuk Peserta (URL Baru):</strong><br>
                <p>Bagikan link berikut kepada peserta:</p>
                <a href="<?php echo htmlspecialchars($pretty_url); ?>" target="_blank"><?php echo htmlspecialchars($pretty_url); ?></a>
            </div>
            <?php endif; ?>
        <?php endif; ?>
